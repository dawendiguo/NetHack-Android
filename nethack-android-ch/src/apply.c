...
struct obj *obj;
{
    ...
    if ((can = mksobj(TIN, FALSE, FALSE)) != 0) {
        ...
        can = hold_another_object(can, "你制作, 但不能拿着, %s.",
    ...
    if (mtmp) {
        ...
        case OBJ_MINVENT:
            if (cansee_spot && !silent && !suppress_see) {
                ...
                You_see("%s %s出%s%s!", monnambuf,
                        locomotion(mtmp->data, "掉落"), carriedby,
                        and_vanish);
            }
            break;
#if 0
            ...
            if (gotit) {
                ...
                case 3:
                    ...
                    otmp = hold_another_object(
                        otmp, "你扔掉%s!", doname(otmp), (const char *) 0);
                    break;
                default:
